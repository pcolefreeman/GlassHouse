...
## Quick Status
- Test count: 266 passing + 12 skipped (2026-03-24) — all tests green
- Exe: no longer used (data collection phase complete)
- Active branch: main
- Unstaged changes: CLAUDE.md
...
- Done: `ov sling` Windows fix (2026-03-24) — 4 patches to installed overstory-cli package (see Overstory section)
...
- Done: Overstory orchestrator first run (2026-03-24) — firmware SAR connectivity merged (stagger rotation, beacon jitter, snap drain, SPIFFS baseline), test coverage branch merged, breathing pipeline needs re-run
- Done: psmux Patch 4 applied + verified (2026-03-25) — list-panes race fix, lead session survived 7m12s to completion
- Done: test_breathing.py merge conflict resolved (2026-03-25) — kept both test sets, removed 2 stale tests referencing deleted _raw_amplitude_energy
- Done: Cleaned up stale worktrees (5) and .local test files (2026-03-25)
- Done: Phase 2 breathing pipeline hardening (2026-03-24) — EMA smoothing, temporal filter, per-path adaptive baseline, path diversity corroboration. 266 tests passing, 12 skipped. Merged via coordinator→lead dispatch.
- Done: psmux ghost session root cause identified (2026-03-24) — sessions were on `-L overstory` socket, not default; sessions.db zombie state blocks coordinator; manual DB fix required
- Done: Patch 5 — `ov clean --all` sessions.db fix (2026-03-24) — SQL purge + WAL checkpoint before file deletion; Windows unlink fails silently on WAL-mode DBs, now data is cleared even when file can't be deleted
- Done: Verified booting→working transition works (2026-03-24) — PreToolUse hook triggers `updateLastActivity` which transitions state; root cause of previous failures was stale zombie records in sessions.db blocking clean coordinator start
- Done: Verified metrics.db population (2026-03-24) — session-end hook correctly writes session metrics; was previously empty because session lifecycle was broken by zombie state
- Done: Patch 6 — OVERSTORY_AGENT_NAME env var fix for Windows (2026-03-25) — hooks-deployer split guard: MARKER_GUARD (file-based fallback) for template hooks, ENV_GUARD (env-var only) for capability guards
- Next session: Run a full coordinator objective (e.g., breathing pipeline or test coverage) to validate end-to-end orchestration
