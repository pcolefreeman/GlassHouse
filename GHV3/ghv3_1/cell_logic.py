# ghv3_1/cell_logic.py
"""cell_logic.py — Pure helper functions for the GHV3.1 data collection UI.
...
def first_cell(selected: set[tuple[int, int]]) -> tuple[int, int]:
    """Return the first selected cell in row-major order, or (-1, -1) if none."""
    if not selected:
        return (-1, -1)
    return min(selected)
