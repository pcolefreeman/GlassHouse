"""serial_io.py — bidirectional CSI data collection with RSSI ranging.

Threads:
  SerialReader — byte stream → frame_queue  (parses all frame types)
  CSVWriter    — frame_queue → 200ms bucket → CSV row; routes ranging frames
"""
import csv
import datetime
import logging
...
from ghv3_1.config import ACTIVE_SHOUTER_IDS, BUCKET_MS, BAUD_RATE

_log = logging.getLogger("ghv3_1.serial_io")
...
class SerialReader(threading.Thread):
    ...
    def _read_one_frame(self):
        ...
        elif b0 == 0xCC:                          # ← NEW: must be before 0x20
            b1 = self._ser.read(1)
            if not b1 or b1[0] != 0xDD:
                return
            payload = self._ser.read(12)
            if len(payload) < 12:
                return
            frame = {
                'magic':       (0xCC, 0xDD),
                'reporter_id': payload[1],        # byte 1 of payload = reporter_id
                'payload':     payload,
            }
            self._queue.put(('ranging', frame))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="GHV3 data collection")
    ...
    if args.output != "data/processed/capture.csv" and args.width is not None and args.depth is not None:
        print(f"[GHV3] NOTE: --output filename ignored when --width/--depth given")
    ...
    print(f"[GHV3] {args.port}  →  {output_path}")
    print(f"[GHV3] label={args.label}  zone={args.zone}  row={args.row}  col={args.col}")
    if args.width is not None and args.depth is not None:
        print(f"[GHV3] area={args.width:.1f}m × {args.depth:.1f}m")
    if args.duration > 0:
        print(f"[GHV3] auto-stop after {args.duration:.0f}s")
    print("[GHV3] Press Ctrl+C to stop\n")
        ...
        try:
            while True:
                ...
                if args.duration > 0 and (time.time() - start_time) >= args.duration:
                    print(f"\n[GHV3] Duration elapsed ({args.duration:.0f}s). Stopping.")
                    break
            ...
            ser.close()
    print(f"[GHV3] Saved to {output_path}")
