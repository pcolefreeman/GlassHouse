"""debug_tab.py — Listener and shouter debug monitors for GHV3.1 UI."""
    ...
    def _connect_listener_debug(self) -> None:
        ...
        try:
            ...
            self._lst_log_file.write(
                f"GHV3 Listener Debug Log — {port} — {time.strftime('%Y-%m-%d %H:%M:%S')}\n"
                + "=" * 60 + "\n\n"
            )
    ...
    def _connect_shouter_debug(self) -> None:
        ...
        try:
            ...
            self._sht_log_file.write(
                f"GHV3 Shouter Debug Log — {port} — {time.strftime('%Y-%m-%d %H:%M:%S')}\n"
                + "=" * 60 + "\n\n"
            )
