"""Tests for ghv4.pi_display — layout, state, demo thread, queue drain."""
import queue
import threading
import time
...
from ghv4.config import CELL_LABELS, PI_SCREEN_SIZE

# pygame may not be installed in all test environments
pygame = pytest.importorskip("pygame")

from ghv4.pi_display import GridDisplay, DemoThread  # noqa: E402
...
# ---------------------------------------------------------------------------
# Confidence extraction (unit test for predict_proba vs predict)
# ---------------------------------------------------------------------------
class TestConfidenceExtraction:

    def test_predict_proba_path(self):
        """Model with predict_proba should yield real confidence."""
        import numpy as np
        ...
        result_proba = mock_model.predict_proba([[0.0] * 100])[0]
        cell_idx = int(result_proba.argmax())
        cell = CELL_LABELS[cell_idx]
        confidence = float(result_proba[cell_idx])

        assert cell == "r1c1"
        assert confidence == pytest.approx(0.87)

    def test_predict_fallback(self):
        """Model without predict_proba should fall back to predict with confidence=1.0."""
        mock_model = MagicMock(spec=['predict'])  # no predict_proba
        mock_model.predict.return_value = ["r2c0"]

        assert not hasattr(mock_model, 'predict_proba')
        prediction = mock_model.predict([[0.0] * 100])
        cell = prediction[0]
        confidence = 1.0
