...
def _make_synthetic_H(tau_true: float, K: int, N: int,
    ...
    """
    from ghv5.spacing_estimator import SUBCARRIER_FREQS
...
def _make_csi_bytes_for_tau(tau: float) -> bytes:
    ...
    from ghv5.spacing_estimator import SUBCARRIER_FREQS, VALID_INDICES
...
def test_collect_accumulates_snapshots():
    from ghv5.spacing_estimator import CSIMUSICEstimator
    from ghv5.config import MUSIC_MIN_SNAP as MIN_SNAP
    from scipy.constants import c as C
    est = CSIMUSICEstimator()
    csi = _make_csi_bytes_for_tau(7.1 / C)
