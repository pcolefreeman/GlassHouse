...
/* GlassHouse v2: Multi-MAC peer whitelist for pairwise CSI sensing. */
...
static uint32_t s_mac_drop_count = 0;  /* Audit SR-1: MAC filter drop counter */
...
static void wifi_csi_callback(void *ctx, wifi_csi_info_t *info)
{
    ...
    /* GlassHouse v2: Multi-MAC peer whitelist filter + link reporting.
     * Replaces ADR-060 single MAC filter with pairwise peer matching. */
    if (s_peer_count > 0) {
        int peer_idx = -1;
        ...
        if (peer_idx < 0) {
            /* Audit SR-1: periodic drop counter for integration debugging */
            s_mac_drop_count++;
        ...
        /* Compute mean amplitude + per-subcarrier amplitude vector.
         * Per-subcarrier amps feed the SAR firehose stream (room-independent
         * physiological-band features). Mean-amp keeps link_reporter /
         * sar_amp working as before. */
        int8_t *buf = (int8_t *)info->buf;
        int num_sc = info->len / 2;  /* I/Q pairs */
        /* Defensive cap: the n_subcarriers u16 truncation bug in ADR-018
         * (csi_serialize_frame, this file:104) can produce pathological
         * info->len up to ~32,000 bytes. Iterating 16,000 sqrtf calls
         * inside the WiFi callback would block RX for ~1.3 ms. Cap at
         * 256 subcarriers (512 byte payload) — covers HT20 (64 sc),
         * HT40 (128 sc), and dual-LTF variants with margin. */
        if (num_sc > 256) num_sc = 256;
        float sum_amp = 0;
        uint8_t per_sc_amps[SAR_FIRE_MAX_SUBCAR];
        int sc_out = 0;
        for (int j = 0; j < num_sc; j++) {
            ...
            sum_amp += amp;
            if (sc_out < SAR_FIRE_MAX_SUBCAR) {
                int ia = (int)amp;
                if (ia < 0) ia = 0;
                if (ia > 255) ia = 255;
                per_sc_amps[sc_out++] = (uint8_t)ia;
            }
        }
        float mean_amp = sum_amp / (num_sc > 0 ? num_sc : 1);

        int64_t ts_now_us = esp_timer_get_time();
        link_reporter_record(s_peer_node_ids[peer_idx], mean_amp);
        sar_amp_push(s_peer_node_ids[peer_idx], mean_amp, ts_now_us);
        sar_fire_push(s_peer_node_ids[peer_idx],
                      per_sc_amps, (uint16_t)sc_out,
...
/* ---- ADR-029: Periodic NDP probe driver ---- */
