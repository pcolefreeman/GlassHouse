...
void app_main(void)
{
...
#else
    /* GlassHouse v2: Register peer MACs from NVS before starting CSI. */
    ...
    /* Lane B Fix 2: Activate dormant NDP probe at 50 Hz.
     * Previously csi_inject_ndp_frame() was defined but never called, so
     * observed off-device CSI rate was 0.4-0.8 Hz instead of the 50 Hz
     * allowed by CSI_MIN_SEND_INTERVAL_US. The periodic NDP broadcast
     * stimulates CSI callbacks on every peer at a controlled cadence. */
    csi_collector_start_ndp_probe(20000);  /* 20 ms period = 50 Hz */

    /* GlassHouse v2: Start heartbeat (CSI stimulus) and link reporter. */
    heartbeat_start(g_nvs_config.target_ip, g_nvs_config.target_port, 50);

    /* SAR quick-win fix 4: interval is Kconfig-driven.
     * SAR mode → 50 ms (20 Hz) to match the 50 Hz CSI target.
     * Non-SAR  → 200 ms (preserves historical battery-friendly cadence). */
#if CONFIG_SAR_MODE
    uint32_t link_interval_ms = CONFIG_SAR_LINK_REPORT_INTERVAL_MS;
    ESP_LOGI(TAG, "link_reporter interval: %lu ms (SAR_MODE=1)",
             (unsigned long)link_interval_ms);
#else
    uint32_t link_interval_ms = CONFIG_LINK_REPORT_INTERVAL_MS;
    ESP_LOGI(TAG, "link_reporter interval: %lu ms (SAR_MODE=0)",
             (unsigned long)link_interval_ms);
#endif
    link_reporter_start(g_nvs_config.node_id, g_nvs_config.target_ip,
                        g_nvs_config.target_port, link_interval_ms);

    /* SAR_AMP starvation fix: periodically sweep all peer rings and flush any
     * with stale samples. Without this, peers with <2 pushes/sec never hit
     * the opportunistic timeout check inside sar_amp_push and their rings
     * stall forever. Shipping alongside Option D firehose for completeness. */
    sar_amp_tick_start(500);  /* 500 ms sweep */
