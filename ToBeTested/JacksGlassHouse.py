...
BUCKET_MS    = 500              # aligned with shouter SEND_INTERVAL_MS (500ms)
NUM_SHOUTERS = 3               # match actual active shouters (bump to 4 when HW added)
...
# MIN_FRAMES: minimum number of distinct shouters heard per bucket.
# Set to 1 so sparse buckets still produce rows.  Raise later for
# stricter quality gating once all shouters are confirmed reliable.
MIN_FRAMES = 1
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            # Remove leftover .bin files so they don't contaminate the next run
            stale_bins = glob.glob(os.path.join(BIN_DIR, "*.bin"))
            if stale_bins:
                for f in stale_bins:
                    try:
                        os.remove(f)
                        ...
                        pass
                print(f"  Cleaned {len(stale_bins)} stale .bin file(s) from {BIN_DIR}")
